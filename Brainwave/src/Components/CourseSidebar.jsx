...
function CourseSidebar({ thisCourse }) {
  const { error, isPending, data: courses } = useFetch('http://localhost:3000/courses/');

  const reviews = [
    { id: 1, name: 'João Castro', text: '"Tudo muito bem explicado, o professor está de parabéns!"' },
    { id: 2, name: 'Maria Amélia', text: '"Aulas feitas com bastante cuidado e a pensar na evolução dos alunos!"' },
    { id: 3, name: 'Carlos Lima', text: '"Excelente professor! Sempre disponível para tirar duvidas!"' },
  ];
  ...
  return (
    <div>
        <div className="flex items-center px-4 py-2 border-b">
            ...
            <h4 className="text-lg font-bold">Professor {thisCourse.prof}</h4>
            </div>
            <div className="p-4 border-b">
            <h4 className="text-xl font-bold mb-4">Cursos Similares</h4>
              {!isPending && courses.map(course => (
                (course.subject === thisCourse.subject && course.id !== thisCourse.id) && 
                <Link to={`/TodosCursos/${course.id}`}>
                  <div className="bg-white border border-gray-300 rounded-lg shadow-md flex h-56 w-full relative mb-5 hover:shadow-xl transform hover:scale-105 transition duration-500">
                      ...
                      <div className="flex-1 flex flex-col justify-between p-4">
                          <div>
                              ...
                              <p className="text-gray-700 text-sm mb-4">{ course.mini_description }</p>
                          </div>
                ...
                <div key={review.id} className="card shadow-lg compact bg-base-100 transform scale-120 w-100 h-32">
                  <div className="card-body">
                    <div className="flex flex-row items-center space-x-20">
                      <h2 className="card-title z-10"><img className="w-12 h-12 object-cover rounded-full mr-4" src={image} alt="Imagem do Professor" />{review.name}</h2> 
                      {(review.name === 'João Castro' || review.name === 'Carlos Lima') && 
